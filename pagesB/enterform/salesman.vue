		...
		<div class="res-container">
			<ul class="star">
				...
				<li @click="choseCity(1)">
					<span>请选择城市合伙人：</span>
					<div style="display: flex;align-items: center;">
						<span style="color: #999999;font-weight: 500" v-if="partners==''">请选择城市合伙人</span>
						<span style="color: #999999;font-weight: 500" v-else>{{partners}}</span>
						<view class="iconfont icon-youjiantou"></view>
					</div>
				</li>
				<li>
		...
		methods: {
			choseCity(n) {
				if (n == 0) {
					...
					this.city == '' ? this.$api.tip('请选择城市!') : this.$api.navTo(
						`/pagesB/enterform/partnersList?id=${this.city}`);
				}
			},
			onConfirm(e) {
				this.city = e.label;
			...
			sub() {
				if (this.city == '') {
					...
					this._reqw.ipost(parmas, 'addPartner').then(res => {
