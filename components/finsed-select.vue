<template>
	<div class="show-box"  :class="{disabled: disabled, active: isShowList}" style="style_Container" @click.stop>
		...
		>
		<!-- 显示框 -->
		<div v-else class="input" :class="{placeholder: selectText === placeholder}" @click.stop="onUpperClick" >{{selectText}}</div>
...
<style scoped lang="less">
	...
	.show-box{
		...
		.right-arrow{
			...
			position: absolute; right: 0px; display: flex; 
			top: 0;
		...
		/****** 列表框部分样式 *****/
		.list-container{
			position: absolute; width: 100%; left: 0; top: 70upx;
			box-sizing: border-box;
			z-index: 999;
			//***************************  弹出框上面的小三角  ***************************
			.popper__arrow{
