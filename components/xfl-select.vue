<template>
	<div class="show-box"  :class="{disabled: disabled, active: isShowList}" style="style_Container" @click.stop>
		<!-- 输入框，仅在可输入模式下使用 -->
		<!-- 显示框 -->
		<div  class="input" :class="{placeholder: selectText === placeholder}" @click.stop="onUpperClick" >{{selectText}}</div>

		<!-- 右侧的小三角图标 -->
		<span 
			@click.stop="onUpperClick" 
			class="iconfont icon-xiajiantou right-arrow" 
		...
		<span 
			...
			class="right-arrow" @click.stop="onClear" 
		>
		...
		@click.stop="onListClick" v-show="isShowList">
			<scroll-view 
...
<style scoped lang="less">
	...
	.show-box{
		...
		.right-arrow{
			...
			font-weight: 100;  justify-content: center;
		}
		.clear{
		...
		/****** 列表框部分样式 *****/
		.list-container{
			position: absolute; width: 100%; left: 0; top: 30px;
			box-sizing: border-box;
			z-index: 9999;
			//***************************  弹出框上面的小三角  ***************************
			.popper__arrow{
